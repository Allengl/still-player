...
export class AudioEngine {
    ...
    private statusSubscription: ReturnType<AudioPlayer["addListener"]> | null =
        null;
    ...
    private async initAudioMode() {
        ...
        try {
            ...
            // setAudioModeAsync is not available on web — safe to ignore
        }
    }
    ...
    markB() {
        ...
        if (this.state.currentTime <= (this.state.abMarkers.pointA ?? 0))
            return;
    ...
    adjustA(deltaSeconds: number) {
        ...
        const newA = Math.max(
            0,
            Math.min(abMarkers.pointA + deltaSeconds, duration),
        );
    ...
    activateLockScreen() {
        ...
        try {
            ...
            // setActiveForLockScreen is not available on web — safe to ignore
        }
    }
